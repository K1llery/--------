...
| `backend/tests/test_algorithms.py` | 覆盖 Top-K、Quickselect、搜索索引、图最短路、交通方式限制、自动漫游、多点路线、室内导航、Huffman 和 AIGC 分镜。 |
...
- 路线：`edges.json` 权威道路建模、单点最短路、最近设施到达、自动漫游闭环、距离/时间/拥挤度/交通方式策略、`route_edges` 元数据、单源距离复用、小规模精确闭环、大规模近似闭环。
