...
- 路线：本地有向图 + Dijkstra/A* 单点路线，Held-Karp 与 2-opt 多点闭环路线，支持距离、时间、拥挤度和交通方式策略；北邮样板路网来自项目内 `datasets/source_maps/bupt_campus_map.json`。
...
| 路线规划 | 单目标最短路、多目标闭环、地图展示、路径展示、最短距离、最短时间、拥挤度、交通工具限制、室内导航。 | Dijkstra/A*、Held-Karp、Nearest Neighbor + 2-opt、Leaflet 地图、室内导航服务。 |
